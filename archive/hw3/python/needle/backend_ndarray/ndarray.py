...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        for i, sl in enumerate(idxs):
            new_shape.append(sl.stop - sl.start)
            new_strides.append(self._strides[i] * sl.step)
            new_offset += sl.start * self._strides[i]
        return NDArray.make(new_shape, tuple(new_strides), self._device, self._handle, new_offset)
        '''
        new_shape = [(sl.stop - sl.start + sl.step - 1) // sl.step for sl in idxs]
        offset_list = [sl.start * st for sl, st in zip(idxs, self._strides)]
        offset = 0
