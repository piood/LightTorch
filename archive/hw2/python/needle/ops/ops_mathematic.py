...
class Transpose(TensorOp):
    ...
    def compute(self, a):
        ### BEGIN YOUR SOLUTION
        size = len(a.shape)
        axes_ = [i for i in range(size)]
        if self.axes is None:
            axes_[-1], axes_[-2] = axes_[-2], axes_[-1]
        else:
            axes_[self.axes[0]], axes_[self.axes[1]] = axes_[self.axes[1]], axes_[self.axes[0]]
        return array_api.transpose(a, axes_)
        ### END YOUR SOLUTION
